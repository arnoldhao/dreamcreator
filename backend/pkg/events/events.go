...
func (eb *eventBus) Publish(ctx context.Context, event Event) error {
	...
	for _, wrapper := range handlers {
		...
		if err := eb.handleWithRetry(ctx, wrapper.handler, event); err != nil {
			logger.Error("Failed to handle event %s: %v", zap.String("event_id", event.GetID()), zap.Error(err))
			// 继续处理其他处理器，不因一个失败而中断
		}
	}
...
func (eb *eventBus) PublishAsync(ctx context.Context, event Event) {
	go func() {
		if err := eb.Publish(ctx, event); err != nil {
			logger.Error("Async event publish failed: %v", zap.String("event_id", event.GetID()), zap.Error(err))
		}
	}()
}
