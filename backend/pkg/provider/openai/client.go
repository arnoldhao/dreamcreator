...
func NewClient(baseURL, apiKey string, httpc *http.Client) *Client {
    ...
    if httpc != nil { cfg.HTTPClient = httpc }
    if strings.TrimSpace(baseURL) != "" { cfg.BaseURL = strings.TrimRight(baseURL, "/") }
...
func (c *Client) ListModels(ctx context.Context) ([]string, error) {
    ...
    for _, m := range resp.Models { out = append(out, m.ID) }
    if len(out) == 0 { out = []string{"default"} }
    return out, nil
}
...
func (c *Client) ChatCompletions(ctx context.Context, model string, messages []ChatMessage, temperature float64) (string, error) {
    ...
    for _, x := range messages { m = append(m, oa.ChatCompletionMessage{Role: x.Role, Content: x.Content}) }
...
func (c *Client) ChatCompletionsWithOpts(ctx context.Context, model string, messages []ChatMessage, temperature float64, topP float64, maxTokens int, jsonMode bool) (string, error) {
    ...
    if temperature > 0 { req.Temperature = float32(temperature) }
    if topP > 0 { req.TopP = float32(topP) }
    ...
    for {
        ...
        if len(resp.Choices) > 0 {
            ...
            if delta != "" && onDelta != nil {
                if e := onDelta(delta); e != nil { return e }
            }
        }
    }
}
...
func (c *Client) CreateEmbeddings(ctx context.Context, model string, inputs []string) ([][]float32, error) {
    if len(inputs) == 0 { return [][]float32{}, nil }
    ...
    for _, d := range resp.Data { out = append(out, d.Embedding) }
    return out, nil
}
...
func (c *Client) CreateImageBase64(ctx context.Context, model, prompt, size string, n int) ([][]byte, error) {
    ...
    for _, d := range resp.Data {
        if b, e := base64.StdEncoding.DecodeString(d.B64JSON); e == nil { out = append(out, b) }
    }
    return out, nil
}
