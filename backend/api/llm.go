...
func (api *LLMAPI) ListProviders() types.JSResp {
    ...
    for _, p := range list { outs = append(outs, toPublicProvider(p)) }
...
func (api *LLMAPI) CreateProvider(in providerCreateReq) types.JSResp {
    ...
    if in.RateLimit != nil { rl = *in.RateLimit }
    res, err := api.svc.CreateProvider(api.ctx, provider.CreateProviderInput{
        ...
        InferenceSummary: func() bool { if in.InferenceSummary!=nil { return *in.InferenceSummary }; return false }(),
...
func (api *LLMAPI) UpdateProvider(id string, in providerCreateReq) types.JSResp {
    ...
    if strings.TrimSpace(in.Type) != "" { v := types.ProviderType(in.Type); tPtr = &v }
    if strings.TrimSpace(in.Policy) != "" { v := types.ProviderPolicy(in.Policy); pPtr = &v }
    if in.Platform != "" { v := in.Platform; plat = &v }
    if in.Name != "" { v := in.Name; name = &v }
    if in.BaseURL != "" { v := in.BaseURL; base = &v }
    if in.Region != "" { v := in.Region; region = &v }
    if in.APIKey != "" { v := in.APIKey; key = &v }
    if in.RateLimit != nil { rlPtr = in.RateLimit }
    if in.Enabled != nil { enPtr = in.Enabled }
    if strings.TrimSpace(in.AuthMethod) != "" { v := in.AuthMethod; amPtr = &v }
    if strings.TrimSpace(in.APIVersion) != "" { v := in.APIVersion; avPtr = &v }
    if in.InferenceSummary != nil { isPtr = in.InferenceSummary }
    if in.APIUsage != nil { m := in.APIUsage; auPtr = &m }
    if in.ProjectID != "" { v := in.ProjectID; proj = &v }
    if in.SAEmail != "" { v := in.SAEmail; sa = &v }
    if in.SAPrivateKey != "" { v := in.SAPrivateKey; sak = &v }
...
func (api *LLMAPI) RefreshModels(id string) types.JSResp {
    ...
    ok := err == nil
    if !ok { logger.Warn("refresh models failed", zap.Error(err)) }
