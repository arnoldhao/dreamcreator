...
func (s *Service) EnsureProjectAnalysis(ctx context.Context, proj *types.SubtitleProject, srcLang, providerID, model string, prof *types.LLMProfile) (*types.ProjectAnalysis, provider.TokenUsage, error) {
    ...
    if proj == nil { return nil, tu, fmt.Errorf("project nil") }
    if proj.Metadata.Analysis != nil { return proj.Metadata.Analysis, tu, nil }

    sys, user := BuildProjectAnalysisPrompts(srcLang, func() string {
        if prof != nil { return prof.SysPromptTpl }
        return ""
    }(), proj.Segments)
    ...
    if err != nil { return nil, tu, err }
    tu = u
    payload := strings.TrimSpace(content)
    if strings.HasPrefix(payload, "```") {
        // 容错：去掉 code fence
        if p := strings.Index(payload, "\n"); p > -1 { payload = strings.TrimSpace(payload[p+1:]) }
    ...
    if err := json.Unmarshal([]byte(payload), &parsed); err != nil {
        ...
        if parsed.Genre == "" && len(parsed.StyleGuide) == 0 && len(parsed.SceneOutline) == 0 {
            logger.Warn("analysis: failed to parse JSON", zap.Error(err), zap.String("preview", func() string { if len(payload) > 300 { return payload[:300] + "..." }; return payload }()))
