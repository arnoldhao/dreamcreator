...
func (s *BoltStorage) DeleteProvider(id string) error {
	...
	return s.db.Update(func(tx *bbolt.Tx) error {
		b := tx.Bucket(providersBucket)
		if b == nil {
			return fmt.Errorf("providers bucket missing")
		}
...
func (s *BoltStorage) GetGlobalProfile(id string) (*GlobalProfileRecord, error) {
	...
	err := s.db.View(func(tx *bbolt.Tx) error {
		...
		v := b.Get([]byte(id))
		if v == nil {
			return fmt.Errorf("global_profile not found: %s", id)
		}
		return json.Unmarshal(v, &rec)
	})
	if err != nil {
...
func (s *BoltStorage) GetModelMeta(providerID, model string) (*ModelMetaRecord, error) {
	...
	err := s.db.View(func(tx *bbolt.Tx) error {
		...
		v := b.Get(key)
		if v == nil {
			return fmt.Errorf("model meta not found: %s/%s", providerID, model)
		}
		return json.Unmarshal(v, &out)
	})
	if err != nil {
...
func (s *BoltStorage) ResetLLMData() error {
	return s.db.Update(func(tx *bbolt.Tx) error {
		if err := tx.DeleteBucket(providersBucket); err != nil && err != bbolt.ErrBucketNotFound {
			return err
		}
		if err := tx.DeleteBucket(globalProfilesBucket); err != nil && err != bbolt.ErrBucketNotFound {
			return err
		}
		if err := tx.DeleteBucket(modelsCacheBucket); err != nil && err != bbolt.ErrBucketNotFound {
			return err
		}
		if err := tx.DeleteBucket(modelsMetaBucket); err != nil && err != bbolt.ErrBucketNotFound {
			return err
		}
