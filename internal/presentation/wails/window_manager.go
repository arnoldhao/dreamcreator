...
const chatFileDropTarget = "chat-composer"
...
func (manager *WindowManager) EmitChatNavigate(threadID string) {
	manager.app.Event.Emit("chat:navigate", threadID)
}
...
func (manager *WindowManager) EmitAssistantsUpdated() {
	...
	emit := func() {
		manager.app.Event.Emit("assistants:updated")
	}
	if manager.initialized {
...
func (manager *WindowManager) EmitProvidersUpdated() {
	if manager == nil || manager.app == nil {
		return
	}
	emit := func() {
		manager.app.Event.Emit("providers:updated")
	}
	if manager.initialized {
		application.InvokeSync(emit)
		return
...
func (manager *WindowManager) EmitExternalToolsUpdated() {
	if manager == nil || manager.app == nil {
		return
	}
	emit := func() {
		manager.app.Event.Emit("external-tools:updated")
	}
	if manager.initialized {
		application.InvokeSync(emit)
		return
...
func (manager *WindowManager) EmitMemoryPrincipalAvatarUpdated(payload map[string]any) {
	if manager == nil || manager.app == nil {
		return
	}
	emit := func() {
		manager.app.Event.Emit("memory:principal-avatar-updated", payload)
	}
	if manager.initialized {
		application.InvokeSync(emit)
		return
...
func (manager *WindowManager) registerMainWindowEvents() {
	...
	manager.mainWindow.OnWindowEvent(events.Common.WindowFilesDropped, func(event *application.WindowEvent) {
		if manager == nil || manager.app == nil {
			return
		}
		...
		if details.Attributes["data-file-drop-target"] != chatFileDropTarget {
			return
		}
		...
		attachments := make([]SelectedAttachmentFile, 0, len(paths))
		for _, path := range paths {
			item, err := readSelectedAttachmentFile(path)
			if err != nil {
				zap.L().Warn("chat attachment drop rejected", zap.Error(err))
				manager.app.Event.Emit("chat:attachments-drop-error", err.Error())
				continue
			}
		...
		manager.app.Event.Emit("chat:attachments-dropped", map[string]any{
			"files":   attachments,
			"details": details,
		})
	})
