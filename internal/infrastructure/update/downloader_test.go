...
func TestHTTPDownloaderRetriesWithRangeResume(t *testing.T) {
	...
	payload := bytes.Repeat([]byte("dreamcreator-update-"), 2048)
	half := len(payload) / 2
	...
	path, err := downloader.Download(context.Background(), server.URL+"/dreamcreator.zip", nil)
	if err != nil {
...
func TestHTTPDownloaderRestartsWhenServerIgnoresRangeRequest(t *testing.T) {
	...
	payload := bytes.Repeat([]byte("dreamcreator-portable-"), 1536)
	half := len(payload) / 2
