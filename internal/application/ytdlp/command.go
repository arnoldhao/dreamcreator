...
func BuildSubtitleArgs(request dto.CreateYTDLPJobRequest, outputTemplate string, subtitleTemplate string, cookiesPath string, explicitToolArgs []string, proxyURL string) []string {
	...
	if subtitleFormat := strings.TrimSpace(request.SubtitleFormat); subtitleFormat != "" {
...
func buildCommand(ctx context.Context, options CommandOptions, subtitleOnly bool) (Command, error) {
	...
	if execPath == "" {
		...
		resolved, err := options.Tools.ResolveExecPath(ctx, externaltools.ToolYTDLP)
		if err != nil {
			return Command{}, err
		}
	...
	printFilePath := ""
	cleanup := func() {}
	if printFile, err := os.CreateTemp("", "dreamcreator-ytdlp-output-*.txt"); err == nil {
