...
func TestFinalizeUsesCachedCookiesWhenCDPUnavailable(t *testing.T) {
	...
	connector, err := connectors.NewConnector(connectors.ConnectorParams{
		ID:        "connector-google",
		Type:      string(connectors.ConnectorGoogle),
	...
	service := NewConnectorsService(repo, nil)
	...
	session := &connectorSession{
		...
		LastCookies:       []appcookies.Record{{Name: "SID", Value: "1", Domain: ".google.com", Path: "/"}, {Name: "HSID", Value: "2", Domain: ".google.com", Path: "/"}},
		LastCookiesAt:     now,
