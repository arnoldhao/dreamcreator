...
func (service *LibraryService) CheckYTDLPOperationFailure(ctx context.Context, request dto.CheckYTDLPOperationFailureRequest) (dto.CheckYTDLPOperationFailureResponse, error) {
	...
	toolNames := []externaltools.ToolName{externaltools.ToolYTDLP, externaltools.ToolFFmpeg, externaltools.ToolBun}
	...
	for _, item := range items {
		if item.Status != ytdlpCheckStatusOK && (item.ID == "connectivity" || item.ID == string(externaltools.ToolYTDLP) || item.ID == string(externaltools.ToolFFmpeg) || item.ID == string(externaltools.ToolBun)) {
			canRetry = false
			break
		}
	}
...
func (service *LibraryService) checkYTDLPTool(ctx context.Context, name externaltools.ToolName) (string, string) {
