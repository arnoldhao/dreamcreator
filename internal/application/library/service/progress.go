...
func (service *LibraryService) publishFileDelete(id string) {
	trimmed := strings.TrimSpace(id)
	if trimmed == "" {
		return
	}
	service.publishEvent(libraryTopicFile, libraryEventDelete, map[string]string{"id": trimmed})
}
...
func (service *LibraryService) publishWorkspaceProjectUpdate(libraryID string) {
	trimmed := strings.TrimSpace(libraryID)
	if trimmed == "" {
		return
	}
	service.publishEvent(libraryTopicWorkspaceProject, libraryEventUpsert, map[string]string{"libraryId": trimmed})
}
