...
func (service *LibraryService) downloadYTDLPThumbnailPrefetch(
	...
	targetDir := filepath.Join(baseDir, ".thumbnail-prefetch", sanitizeFileName(strings.TrimSpace(operationID)))
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		return "", err
	}
	return service.downloadYTDLPThumbnailToTarget(ctx, thumbnailURL, filepath.Join(targetDir, "thumbnail.jpg"), true)
}
...
func resolveYTDLPPrefetchBaseDir(outputTemplate string) string {
	...
	return filepath.Dir(templateDir)
}
