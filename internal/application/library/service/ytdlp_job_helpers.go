...
import (
	...
	ydlpinfr "dreamcreator/internal/infrastructure/ytdlp"
)
...
func (prefetch *ytdlpThumbnailPrefetch) Start(
	...
	go func() {
		path, err := service.downloadYTDLPThumbnailPrefetch(ctx, request, outputTemplate, operationID)
		prefetch.mu.Lock()
		prefetch.tempPath = strings.TrimSpace(path)
		prefetch.err = err
		prefetch.mu.Unlock()
		close(done)
	}()
...
func (service *LibraryService) runYTDLPOperation(ctx context.Context, operation library.LibraryOperation, history library.HistoryRecord, request dto.CreateYTDLPJobRequest) {
	...
	execPath, err := service.resolveTool(ctx, externaltools.ToolYTDLP)
	if err != nil {
	...
	result, runErr := service.executeYTDLPCommand(operation, command, reporter, func(_ string, line string) {
		...
		reporter.WithOperationLock(func() {
			...
			thumbnailPrefetch.Start(context.Background(), service, request, outputTemplate, operation.ID)
		})
	})
	if runErr != nil {
		...
		service.failYTDLPOperation(ctx, &operation, &history, failureErr, errorCode, buildOperationOutputPayload("", nil, nil, nil, metadataPayload, logSnapshot))
		return
	}
	...
	if prefetchedThumbnailPath, err := thumbnailPrefetch.Wait(); err != nil {
		auxiliaryWarnings = append(auxiliaryWarnings, fmt.Sprintf("thumbnail prefetch failed: %v", err))
		...
		if promotedPath, promoteErr := service.promotePrefetchedYTDLPThumbnail(prefetchedThumbnailPath, outputPath); promoteErr != nil {
			auxiliaryWarnings = append(auxiliaryWarnings, fmt.Sprintf("thumbnail download failed: %v", promoteErr))
	...
	if wantsYTDLPSubtitles(request) {
		if step := service.downloadYTDLPSubtitles(ctx, reporter, execPath, request, outputTemplate, subtitleTemplate, cookiesPath); step.warning != "" {
	...
	operation.OutputJSON = buildOperationOutputPayload(outputPath, afterMovePaths, outputSnapshot.outputPaths, outputSnapshot.outputFiles, metadataPayload, logSnapshot)
	history.Status = string(operation.Status)
	history.DisplayName = operation.DisplayName
	...
	if strings.TrimSpace(request.TranscodePresetID) != "" {
		_, _ = service.CreateTranscodeJob(context.Background(), dto.CreateTranscodeJobRequest{
...
func (service *LibraryService) resolveTool(ctx context.Context, name externaltools.ToolName) (string, error) {
...
func (service *LibraryService) persistYTDLPLogs(ctx context.Context, operation library.LibraryOperation, result ydlpinfr.RunResult, persistOnFailure bool, persistOnSuccess bool, runErr error) appytdlp.LogSnapshot {
	sortedLogs := ydlpinfr.SortLogs(result.Logs)
	shouldPersist := (runErr != nil && persistOnFailure) || (runErr == nil && persistOnSuccess)
	if !shouldPersist || len(sortedLogs) == 0 {
		return appytdlp.LogSnapshot{}
	}
	downloadDir, err := service.resolveDownloadDirectory(ctx)
	if err != nil {
		return appytdlp.LogSnapshot{}
	}
	resolvedPath, err := ydlpinfr.BuildLogPath(downloadDir, operation.ID)
	if err != nil {
		return appytdlp.LogSnapshot{}
	}
	size, err := ydlpinfr.WriteLogFile(resolvedPath, sortedLogs)
	if err != nil {
		return appytdlp.LogSnapshot{}
	}
	return appytdlp.LogSnapshot{Path: resolvedPath, SizeBytes: size, LineCount: len(sortedLogs)}
}
...
func (service *LibraryService) prepareYTDLPOutput(ctx context.Context) (string, string, string, error) {
	...
	baseDir := filepath.Join(downloadDirectory, "dreamcreator", "yt-dlp")
	if err := os.MkdirAll(baseDir, 0o755); err != nil {
		return "", "", "", err
	}
	outputTemplate := filepath.Join(baseDir, "%(extractor)s", "%(uploader)s-%(title)s-%(format_note)s-%(id)s.%(ext)s")
	subtitleTemplate := filepath.Join(baseDir, "%(extractor)s", "subs", "%(uploader)s-%(title)s-%(id)s.%(ext)s")
...
func (service *LibraryService) buildYTDLPOutputs(ctx context.Context, request dto.CreateYTDLPJobRequest, operation library.LibraryOperation, primaryFile library.LibraryFile, started time.Time, outputPath string, resolvedAfterMove string, outputPaths []string, subtitleLogPaths []string, thumbnailPaths []string) (ytdlpOutputSnapshot, error) {
	...
	subtitleDirs := []string{filepath.Dir(outputPath), filepath.Join(filepath.Dir(outputPath), "subs"), filepath.Join(filepath.Dir(outputPath), "subtitles")}
	...
	for _, candidate := range candidatePaths {
		...
		subtitleDirs = append(subtitleDirs, dir, filepath.Join(dir, "subs"), filepath.Join(dir, "subtitles"))
	...
	for _, subtitlePath := range ydlpinfr.FindSubtitleOutputs(outputPath, started, resolveAssetPathFallback, subtitleDirs...) {
		resolvedSubtitlePaths = append(resolvedSubtitlePaths, subtitlePath)
	}
...
func (service *LibraryService) createDownloadedPrimaryFile(ctx context.Context, operation library.LibraryOperation, request dto.CreateYTDLPJobRequest, title string, outputPath string, createdAt time.Time) (library.LibraryFile, error) {
	...
	if err := service.renameLibraryFromFirstFileIfNeeded(ctx, operation.LibraryID, fileItem.Name, createdAt); err != nil {
		return library.LibraryFile{}, err
	}
	return fileItem, nil
}
...
func (service *LibraryService) createDownloadedSubtitleFile(ctx context.Context, operation library.LibraryOperation, path string, baseTitle string, createdAt time.Time) (library.LibraryFile, error) {
	...
	fileItem, err := library.NewLibraryFile(library.LibraryFileParams{
		...
		Name:              strings.TrimSpace(title),
		Storage:           library.FileStorage{Mode: "db_document", DocumentID: documentID},
		Origin:            library.FileOrigin{Kind: "download", OperationID: operation.ID},
		LatestOperationID: operation.ID,
	...
	if err := deleteLocalFileIfExists(path); err != nil {
		fileItem.Storage = library.FileStorage{Mode: "hybrid", LocalPath: path, DocumentID: documentID}
		fileItem.UpdatedAt = service.now()
		if saveErr := service.files.Save(ctx, fileItem); saveErr != nil {
			return library.LibraryFile{}, saveErr
		}
	}
	return fileItem, nil
}
