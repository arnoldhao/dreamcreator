...
export class WebSocketClient {
  ...
  private handleMessage(event: MessageEvent<string>) {
    ...
    const isGatewayEvent = parsed?.type === "event" && parsed?.event;
    const topic = parsed?.topic ?? parsed?.Topic ?? (isGatewayEvent ? String(parsed.event) : "");
    if (!topic) {
      return;
    }
    ...
    const realtimeEvent: RealtimeEvent = {
      ...
      topic: String(topic),
      type: isGatewayEvent ? parsed?.event : parsed?.type ?? parsed?.Type,
