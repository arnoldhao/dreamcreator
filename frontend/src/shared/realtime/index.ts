...
import {
  requestGateway as requestGatewayControlPlane,
...
async function resolveWebSocketURL(): Promise<string> {
  try {
    const url = await Call.ByName("dreamcreator/internal/presentation/wails.RealtimeHandler.WebSocketURL");
...
export const requestGateway = requestGatewayControlPlane;
export { subscribeGatewayEvents, startGateway };
export type { GatewayEvent, GatewayStatus };
