...
export type TaskDeleteFileItem = {
  id?: string
  name?: string
  path?: string
  fileType?: string
...
type LibraryTaskContextMenuProps = {
...
type LibraryTaskDeleteDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  itemName: string
  deleteFileItems?: TaskDeleteFileItem[]
...
export function LibraryTaskContextMenu({
  anchor,
  onClose,
  onView,
  onDelete,
  ...
  return createPortal(
    ...
    >
      <button type="button" role="menuitem" className={itemClassName} onClick={onView}>
...
export function LibraryTaskDeleteDialog({
  open,
  onOpenChange,
  itemName,
  deleteFileItems,
  ...
  const resolvedDeleteFileItems = React.useMemo(
    () =>
      (deleteFileItems ?? []).filter(
        (item): item is TaskDeleteFileItem => Boolean(item && (item.name || item.path))
      ),
    [deleteFileItems]
  )
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {deleteFiles
                        ? t("library.task.deleteFilesEnabled")
                        : t("library.task.deleteFilesDisabled")}
                    </div>
                  </div>
...
function resolveDeleteItemFormat(item: TaskDeleteFileItem) {
